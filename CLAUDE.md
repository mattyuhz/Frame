...
- **Looping must not lag.** A clip whose window is the whole clip gets the browser's native `loop` — its internal wrap costs nothing, where any hand-rolled wrap costs at least a decode. Sub-window clips are wrapped by `wrapCheck()`, which runs *every animation frame* (cheap property reads only) and seeks while the element is still playing, so there is no `play()` to wait for; it also wraps `WRAP_LEAD` (60 ms) early so the element never reaches `ended` — an ended element is paused, and resuming costs a restart instead of a seek. Measured at the loop point: **150–167 ms → 33 ms**, one frame of a 30 fps source, which is the floor. Native `loop` is forced off during export so each window plays through exactly once.
